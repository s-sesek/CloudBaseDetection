...
flags.DEFINE_float('base_learning_rate', .007, 'The base learning rate for model training.')  # change
...
flags.DEFINE_integer('training_number_of_steps', 5, 'The number of steps used for training')
...
flags.DEFINE_float('weight_decay', 0.0001, 'The value of the weight decay for training.')
flags.DEFINE_multi_integer('train_crop_size', [482, 480], 'Image crop size [height, width] during training.')  # change
...
flags.DEFINE_boolean('initialize_last_layer', False, 'Initialize the last layer.')  ##change
...
def checkpointcheck():
    # Instead of having to make changes to the file while retraining
    # attempt to verify if the requested check point exists
    # if it does, set the checkpoint to None so it will begin creating
    # new check points
    check_point_file = FLAGS.tf_initial_checkpoint + ".index"

    # I opted for the any & starts with if we end up not having the .index extension for some reason
    if any(File.startswith(check_point_file) for File in os.listdir(FLAGS.train_logdir)):
        print("Initial check point file found: ", check_point_file)
    else:
        print("Unable to find checkpoint file [", check_point_file, "] in directory:[", FLAGS.train_logdir, "]")
        print("Defaulting to FLAGS.tf_initial_checkpoint=None")
        print("New checkpoints will be created")
...
def main(unused_argv):
    ...
    moddiv = FLAGS.train_batch_size % config.num_clones
    assert moddiv == 0, (
    ...
    dataset = segmentation_dataset.get_dataset(FLAGS.dataset, FLAGS.train_split, dataset_dir=FLAGS.dataset_dir)
    ...
    with tf.Graph().as_default() as graph:
        ...
        session_config = tf.ConfigProto(
            ...
            log_device_placement=True)  # change allow_soft_placement to false #also log_device_placement to true will prob have to change back
