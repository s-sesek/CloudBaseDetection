...
flags.DEFINE_string('vis_logdir', './results', 'Where to write the event logs.')
...
flags.DEFINE_multi_integer('vis_crop_size', [320, 240], 'Crop size [height, width] for visualization.')
...
flags.DEFINE_enum('colormap_type', 'clouds', ['clouds'], 'Visualization colormap type.')

flags.DEFINE_boolean('also_save_raw_predictions', True, 'Also save raw predictions.')
...
def save_annotation(label,
                    ...
                    filename,
                    add_colormap=True,
                    colormap_type=dataset_colormap.get_clouds_name()):
  ...
  if add_colormap:
    colored_label = dataset_colormap.label_to_color_image(
        label, colormap_type)
  else:
    colored_label = label
...
def main(unused_argv):
  ...
  if dataset.name == dataset_info.get_cityscapes_dataset_name():
