...
tf.app.flags.DEFINE_string(
    'train_input_folder',
    '/data/scripts/LES_TSI/SmallData/inputs/train/',  #change
    'Folder containing training images')
tf.app.flags.DEFINE_string(
    'train_target_folder',
    '/data/scripts/LES_TSI/SmallData/targets/train/',  #change
    'Folder containing annotations for training images')

tf.app.flags.DEFINE_string(
    'test_input_folder',
    '/data/scripts/LES_TSI/SmallData/inputs/test/', #change
    'Folder containing validation images')

tf.app.flags.DEFINE_string(
    'test_target_folder',
    '/data/scripts/LES_TSI/SmallData/targets/test/', #change
    'Folder containing annotations for validation')

tf.app.flags.DEFINE_string(
    'output_dir', '/data/scripts/LES_TSI/SmallData/tfrecord',      #change
  ...
  for shard_id in range(_NUM_SHARDS):
    ...
    with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
      ...
      for i in range(start_idx, end_idx):
        ...
        image_data = tf.gfile.FastGFile(image_filename, 'rb').read()   #change make sure 'rb' or utf8 error
        height, width = image_reader.read_image_dims(image_data)
        # Read the semantic segmentation annotation.
        seg_filename = seg_names[i]
        seg_data = tf.gfile.FastGFile(seg_filename, 'rb').read()  #change make sure 'rb' or utf8 error
...
def main(unused_argv):
  ...
  _convert_dataset(
      'train', FLAGS.train_input_folder, FLAGS.train_target_folder)
  _convert_dataset('val', FLAGS.test_input_folder, FLAGS.test_target_folder)
